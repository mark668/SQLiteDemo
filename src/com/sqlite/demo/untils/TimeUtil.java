...
public class TimeUtil{
  ...
  public static String getNowBeginThree(){
      ...
      return calendar.get(GregorianCalendar.YEAR) + "-" + month + "-" + new java.util.Date().getDate();
  }
  ...
  public   static   void  calcToday(String begin,String end,String now,GregorianCalendar calendar)  {
   ...
   public static List<Date> getdate(Date date){ 
	   ...
	   c.add(java.util.Calendar.DAY_OF_WEEK,-today+i+1); 
	   c.getTime(); 
	   list.add(c.getTime()); 
	   c.setTime(date); 
	   } 
	   ...
	   c.add(java.util.Calendar.DAY_OF_WEEK,i); 
	   c.getTime(); 
	   list.add(c.getTime()); 
	   c.setTime(date); 
	   } 
   ...
   public static int getDayFromCookiePlantTime(Date fromDate) {//今天明天昨天
	      Calendar cal = Calendar.getInstance();
   ...
   public static String getDatesNow(String fromDate){
	   int date = getDayFromCookiePlantTime(TimeUtil.parseDateTime2(fromDate));
	   DateFormat df = new SimpleDateFormat("HH:mm");
	   DateFormat df2 = new SimpleDateFormat("yy-MM-dd");
	   df.format(TimeUtil.parseDateTime2(fromDate));
	   if (date == 1) {
		   return "昨天 " + df.format(TimeUtil.parseDateTime2(fromDate));
	   }else if(date == 2){
		   return "前两天 "+df.format(TimeUtil.parseDateTime2(fromDate));
	   }else if(date == 0){
		   return "今天 "+ df.format(TimeUtil.parseDateTime2(fromDate));
	   }else{
		   return ""+df2.format(TimeUtil.parseDateTime2(fromDate));
	   }
   }
